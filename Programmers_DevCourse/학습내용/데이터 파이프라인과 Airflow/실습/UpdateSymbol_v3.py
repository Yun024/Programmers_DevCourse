...
def load(schema, table, records):
    ...
    try:
        ...
        for row in describe_result:
            logging.info(row)
        for r in records:
            sql = f"INSERT INTO t(date, open, high, low, close, volume, created_date) VALUES ('{r[0]}', {r[1]}, {r[2]}, {r[3]}, {r[4]}, {r[5]}, GETDATE());"
            print(sql)
            cur.execute(sql)
        ...
        SELECT date, open, high, low, close, volume, GETDATE() FROM(
...
with DAG(
    ...
    load("yeojun", "stock_info_v3", results)
